...
const closestValue = (numbers: number[], target: number) => {
    let start = numbers[0];
    ...
    while (start <= end) {
        ...
        const distanceFromClosest = Math.abs(target - numbers[closestIdx]);
        const distanceFromMid = Math.abs(target - numbers[mid]);

        if (
            distanceFromClosest > distanceFromMid ||
            (distanceFromClosest === distanceFromMid &&
                numbers[closestIdx] > numbers[mid])
        ) {
            closestIdx = mid;
        }
